...
bool baillie_psw(const mpz_class p) {
    if (!prime_trial_1000(p))
        return false;
    if (p < f_k_primes[firstKPrimesSize - 1] * f_k_primes[firstKPrimesSize - 1])
        return true;
    if (!miller_rabin(p))
        return false;
...
bool prime_trial_1000(const mpz_class p) {
    if ((p & 1) == 1 && p < 1000) {
        return std::binary_search(f_k_primes, f_k_primes + firstKPrimesSize, p);
    }
    for (size_t i = 0; i < firstKPrimesSize; i++) {
        if (p % f_k_primes[i] == 0)
            return false;
    }
    return true;
